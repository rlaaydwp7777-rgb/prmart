...
export type AuthState = {
  ...
  user?: any;
  issues?: string[];
  type?: 'signin' | 'signup' | 'reset' | 'general';
};
...
export async function signUpWithEmailAction(prevState: AuthState, formData: FormData): Promise<AuthState> {
  ...
  if (!emailValidation.success) {
    return { success: false, message: "유효성 검사 실패", issues: emailValidation.error.flatten().formErrors, type: 'signup' };
  }

  const passwordValidation = passwordSchema.safeParse(password);
  if (!passwordValidation.success) {
    return { success: false, message: "유효성 검사 실패", issues: passwordValidation.error.flatten().formErrors, type: 'signup' };
  }
  ...
  try {
    ...
    return { success: true, message: "회원가입이 완료되었습니다! 잠시 후 이동합니다.", type: 'signup' };
  } catch (error: any) {
    console.error("SignUp Error:", error.code, error.message);
    return { success: false, message: getFirebaseAuthErrorMessage(error.code), type: 'signup' };
  }
}
...
export async function signInWithEmailAction(prevState: AuthState, formData: FormData): Promise<AuthState> {
  ...
  try {
    ...
    return { success: true, message: "로그인 성공! 잠시 후 이동합니다.", type: 'signin' };
  } catch (error: any) {
...
export async function signOutAction(): Promise<AuthState> {
    try {
        ...
        return { success: true, message: "로그아웃 되었습니다.", type: 'general' };
    } catch (error: any) {
        console.error("SignOut Error:", error);
        return { success: false, message: "로그아웃 중 오류가 발생했습니다.", type: 'general' };
    }
}
