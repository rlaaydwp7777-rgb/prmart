...
export default function SellerLayout({ children }: { children: React.ReactNode; }) {
  ...
  const NavLink = ({ item }: { item: { href: string, icon: React.ElementType, title: string } }) => (
    <Link
        href={item.href}
        className={cn(
            "flex items-center gap-3 rounded-lg px-4 py-2.5 text-base font-medium transition-all hover:bg-sidebar-accent hover:text-sidebar-accent-foreground",
            pathname === item.href ? "bg-sidebar-accent text-sidebar-accent-foreground" : "text-sidebar-foreground/70"
        )}
     ...
     <div className="flex flex-col h-full bg-sidebar-background text-sidebar-foreground">
        <div className="p-4 border-b border-sidebar-border">
            <Link href="/" className="flex items-center gap-2 font-semibold">
                ...
                <span className="font-bold text-xl font-headline tracking-tight text-sidebar-foreground">prmart</span>
            </Link>
        </div>
