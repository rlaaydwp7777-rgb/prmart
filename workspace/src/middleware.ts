...
export async function middleware(req: NextRequest) {
  ...
  if (isAdminRoute || isSellerRoute || isAccountRoute) {
    ...
    try {
      ...
      // Log the specific error for debugging and security auditing
      console.error(`[MW_TOKEN_VERIFY_FAIL] Token verification failed for path '${pathname}'. Reason: ${err.code}. Redirecting to login.`);
      // If token is expired or invalid, redirect to login, preserving the original destination
      return NextResponse.redirect(loginUrl);
    }
  }
